...
func Test_backspaceCompareTwoPointers(t *testing.T) {
	type args struct {
		s string
		t string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := backspaceCompareTwoPointers(tt.args.s, tt.args.t); got != tt.want {
				t.Errorf("backspaceCompareTwoPointers() = %v, want %v", got, tt.want)
			}
		})
	}
}
